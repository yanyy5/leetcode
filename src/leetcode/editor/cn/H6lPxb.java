...
// Related Topics 深度优先搜索 广度优先搜索 并查集 字符串 👍 5 👎 0
...
class Solution {
    public int numSimilarGroups(String[] strs) {
        ...
        for (int i = 0; i < strs.length; i++){
            for (int j = i+1; j < strs.length; j++){
                if (compareStrs(strs[i], strs[j]) && union(fathers, i, j)){
                    count--;
                }
            }
        }
    ...
    private int findFathers(int[] fathers, int i){
        if (fathers[i] != i){
    ...
    private boolean union(int[] fathers, int i, int j){
        int fi = findFathers(fathers, i);
        int fj = findFathers(fathers, j);
        if (fi != fj){
            fathers[fi] = fj;
            return true;
        }
